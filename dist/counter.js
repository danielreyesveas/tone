var _statcounter = (function (_1) {
	...
	function is_admin_project(_3) {
		return (
			[
				12225189, 11548023, 11878871, 12214659, 981359, 9560334,
				6709687, 9879613, 4124138, 204609, 10776808, 11601825,
			].indexOf(_3) !== -1
		);
	}
	try {
		...
		if (
			...
			document.currentScript.src.indexOf("statcounter.com") !== -1
		) {
		...
		function need_project_config(_12) {
			...
			try {
				...
				if (
					_sessionStorageGetConfig("sc_project_config_" + _12) ===
						1 &&
					_sessionStorageGetConfig(
						"sc_project_time_difference_" + _12
					) !== null
				) {
					ret = false;
				}
					...
					if (typeof performance !== "undefined") {
						...
						for (var i = 0; i < _23.length; i++) {
							...
							if (
								...
								_25.name.includes(
									"statcounter.com/counter/counter_test.js"
								)
							) {
								_20 =
									"&sc_rum_f_s=" +
									Math.round(_25.requestStart) +
									"&sc_rum_f_e=" +
									Math.round(_25.responseEnd);
								break;
							}
						}
					}
		...
		function strip_tags(_32, _33) {
			_33 = (
				((_33 || "") + "").toLowerCase().match(/<[a-z][a-z0-9]*>/g) ||
				[]
			).join("");
			...
			if (!(_3a.length % 2 == 0)) {
				...
				for (var i = 0; i < _3a.length; i++) {
					if (strip_tags("" + _3a[i]) != "" + _3a[i]) {
						_3e.push(
							"Tag names and values may not contain HTML tags."
						);
					}
				}
			}
			...
			if (_42(_41, "tags") && typeof _41.tags === "object") {
				...
				if (validate_tags(_49)) {
					...
					for (var i = 0; i < _49.length; i = i + 2) {
						_48["sc_ev_" + encodeURIComponent(_49[i])] =
							encodeURIComponent(_49[i + 1]);
					}
				}
			}
			return _48;
		...
		function mixkey(_6d, key) {
			...
			while (j < _6f.length) {
				key[_53 & j] =
					_53 & ((_70 ^= key[_53 & j] * 19) + _6f.charCodeAt(j++));
			}
			return tostring(key);
		}
		...
		function detectBrowserFeatures() {
			...
			var _78 = new RegExp("Mac OS X.*Safari/").test(navigator.userAgent)
			...
			if ("localStorage" in window) {
				try {
					...
					if (
						...
						if (
							!e.number ||
							parseInt(e.number, 10) !== -2147024891
						) {
							throw e;
						}
					}
		...
		function _getConfigDataStructureFromSession() {
			...
			try {
				if (sessionStorage.getItem("statcounter_config") !== null) {
					_ac = JSON.parse(
						sessionStorage.getItem("statcounter_config")
					);
				}
			} catch (ignore) {
				_ac = {};
		...
		var _b3 = (function () {
			...
			return function (_b5) {
				...
				if (
					!_sessionStorageGetConfig(
						"sc_project_time_difference_" + _b5
					)
				) {
					return;
				}
		...
		function config_ping(_bf, _c0, _c1) {
			_4.get_config(_c0, function (_c2) {
				...
				_sessionStorageSetConfig(
					"sc_project_time_difference_" + _bf,
					parseInt(_c2["time_difference"])
				);
		...
		function check_root_domains_match(a, b) {
			...
			if (
				...
				((_c8[_c8.length - 2].length <= 3 &&
					...
					_c8[_c8.length - 1] == "ru" ||
					(_c8[_c8.length - 1] == "au" &&
						_c8[_c8.length - 2] in { csiro: 1 }) ||
					(_c8[_c8.length - 1] == "at" &&
						_c8[_c8.length - 2] in { priv: 1 }) ||
					(_c8[_c8.length - 1] == "fr" &&
						_c8[_c8.length - 2] in
					...
					(_c8[_c8.length - 1] == "il" &&
						_c8[_c8.length - 2] in { muni: 1 }) ||
					(_c8[_c8.length - 1] == "za" &&
						_c8[_c8.length - 2] in { school: 1 }) ||
					(_c8[_c8.length - 1] == "tr" &&
						_c8[_c8.length - 2] in { name: 1 }) ||
					(_c8[_c8.length - 1] == "uk" &&
						_c8[_c8.length - 2] in { police: 1 }))
			) {
				_cb = 3;
			}
			for (var i = 1; i <= _ca; i++) {
		...
		_4.record_pageview = function (_d9, _da) {
			...
			if (_dc == 1) {
				...
				if (document.webkitVisibilityState == "prerender") {
					...
					gen_counter(_d9, _db, false, {}, { p: 1 });
				} else {
					send_pageview(_d9, _db, { p: 0 });
				}
			}
		};
		var _de = {};
		function send_pageview(_df, _e0, _e1) {
			...
			if (_e != "disabled") {
				...
				try {
					var _e8 = JSON.parse(
						localStorage.getItem("sc_medium_source")
					);
					if (_e8 == null) {
						_e8 = {};
					}
					...
					if (!apply_new_methodology_fixes(_df)) {
						if (
							_e6 == "r" &&
							sessionStorage.getItem("statcounter_exit_domain") ==
								_e5
						) {
							_e5 = "internal";
						}
				...
				if (_f3["session_incremented"]) {
					...
					if (_f4 && !_f4.match(/(^test$|wsdev$|^dev[0-9]*)/)) {
						_e3 = true;
						_sessionStorageSetConfig(
							"sc_project_time_difference_" + _df,
							false
						);
					}
				}
			...
			if (
				Object.prototype.toString.call(_4._pending_tags) ===
				"[object Array]"
			) {
		...
		function gen_counter(_f7, _f8, _f9, _fa, _fb) {
			...
			if (_f8 == "invisible") {
				...
				if (_ff) {
					try {
						var _101 = sessionStorage.getItem(
							"statcounter_pending"
						);
						if (!_101) {
							var _102 = {};
		...
		function sc_clickstat_call(_125, _126) {
			...
			var _129 =
				"ac|co|gov|ltd|me|mod|net|nic|nhs|org|plc|police|sch|com";
			...
			_12f = _130 + "." + _12f;
			var _131 =
				"^https?://(.*)(" + _12f + "|webcache.googleusercontent.com)";
					...
					if (_132.test(_126)) {
						if (dl.test(_126)) {
							...
							if (
								...
								new RegExp(sc_exit_link_detect, "i").test(_126)
							) {
								_133 = 2;
							} else {
								if (_a == 2) {
		...
		function sc_adsense_click(_13f, _140) {
			if (_140.src.match(_139)) {
				...
				var _141 = escape(
					"Google Adsense " + _140.width + "x" + _140.height
				);
			}
			var _142 =
				_b +
			...
			if (_4._add_recording_event) {
				_4._add_recording_event("adsense", { link: unescape(_141) });
			}
		}
		...
		function sc_exitpage(e) {
			...
			if (typeof _13b != "undefined") {
				for (var i = 0; i < ad.length; i++) {
					...
					var adW =
						parseInt(_15b, 10) + parseInt(ad[i].width, 10) + 15;
					var adH =
						parseInt(_15c, 10) + parseInt(ad[i].height, 10) + 10;
			...
			if (_7) {
				if (
					typeof _13d != "undefined" &&
					_13d.id.substring(0, 6) == "aswift"
				) {
		...
		function initiate_click_detection() {
			...
			for (var i = 0; i < _164.length; i++) {
				var _166 = _164[i];
				if (
					typeof _166.hasAttribute === "function" &&
					_166.hasAttribute("href")
				) {
					sc_process_anchor(_166);
				}
			}
		...
		_4.update_cookie = function (_168, _169, _16a) {
			...
			var _16b = _sessionStorageGetConfig(
				"sc_project_time_difference_" + _168
			);
			var ret = {};
					...
					if (parseInt(_176[0], 10) == _168) {
						...
						_171.push(
							"" +
								...
								_d +
								"." +
								_4._returning_values[_168].join(".")
						);
					} else {
						_171.push(_170[i]);
		...
		_4.get_session_num = function (_180) {
			var _181 = _sessionStorageGetConfig(
				"sc_project_time_difference_" + _180
			);
			if (
				_181 != false &&
				_4._session_increment_calculated[_180] &&
				_4._returning_values[_180].length == _14.length
			) {
			...
			if (_181 === null) {
				...
				if (!_4._session_increment_calculated[_180]) {
					return (
						"x-no-session-num-98" + Math.round(1000 * Math.random())
					);
				} else {
					...
					var _189 = new MutationObserver(function () {
						...
						if (document.title != _187 && _18a != _188) {
							for (var _18b in _4._security_codes) {
								...
								if (
									_de[_18c] === undefined ||
									new Date().getTime() - _de[_18c] > 1000
								) {
									setTimeout(
										function (_18d) {
											if (

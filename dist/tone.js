(function (root) {
	...
	mainModule(function () {
		 ...
		 *  @const
		 */
		Tone.prototype.bufferTime =
			Tone.prototype.bufferSize / Tone.context.sampleRate;
		...
		Tone._initAudioContext(function (audioContext) {
			//set the bufferTime
			Tone.prototype.bufferTime =
				Tone.prototype.bufferSize / audioContext.sampleRate;
	...
	toneModule(function (Tone) {
		...
		Tone.Signal.prototype.exponentialRampToValueAtTime = function (
			...
			value *= this._syncRatio;
			try {
				this._scalar.gain.exponentialRampToValueAtTime(
					value,
					this.toSeconds(endTime)
				);
			} catch (e) {
		 ...
		 *  @param  {Tone.Time} endTime
		 */
		Tone.Signal.prototype.exponentialRampToValueNow = function (
			value,
			endTime
		) {
			var now = this.now();
		...
		Tone.Signal.prototype.setTargetAtTime = function (
			...
			value *= this._syncRatio;
			this._scalar.gain.setTargetAtTime(
				value,
		...
		Tone.Signal.prototype.setValueCurveAtTime = function (
			...
			this._scalar.gain.setValueCurveAtTime(
				values,
				this.toSeconds(startTime),
	...
	toneModule(function (Tone) {
		...
		Tone.Pow = function (exp) {
			...
			this._expScaler =
				this.input =
				this.output =
					new Tone.WaveShaper(this._expFunc(exp), 8192);
		};
	...
	toneModule(function (Tone) {
		...
		Tone.Envelope.prototype.set = function (params) {
			...
			if (!this.isUndef(params.exponent))
				this.setExponent(params.exponent);
		};
	...
	toneModule(function (Tone) {
		...
		Tone.Compressor.prototype.set = function (params) {
			if (!this.isUndef(params.attack)) this.setAttack(params.attack);
			if (!this.isUndef(params.release)) this.setRelease(params.release);
			if (!this.isUndef(params.threshold))
	...
	toneModule(function (Tone) {
		...
		Tone.Filter.prototype.set = function (params) {
			...
			if (!this.isUndef(params.detune))
				this.detune.setValue(params.detune);
			if (!this.isUndef(params.frequency))
				this.frequency.setValue(params.frequency);
		...
		Tone.Filter.prototype.setRolloff = function (rolloff) {
			...
			if (cascadingCount % 1 !== 0) {
				throw new RangeError(
					"Filter rolloff can only be -12, -24, or -48"
				);
			}
	...
	toneModule(function (Tone) {
		...
		Tone.EQ.prototype.set = function (params) {
			...
			if (!this.isUndef(params.lowFrequency))
				this.lowFrequency.setValue(params.lowFrequency);
			if (!this.isUndef(params.highFrequency))
				this.highFrequency.setValue(params.highFrequency);
		};
	...
	toneModule(function (Tone) {
		...
		Tone.FeedbackCombFilter = function (minDelay, maxDelay) {
			...
			var delayCount = Math.ceil(
				this.bufferSize / (minDelay * this.context.sampleRate)
			);
			//set some ranges
			 ...
			 *  @private
			 */
			this._resScale = new Tone.ScaleExp(
				0.01,
				1 / this._delayCount - 0.001,
				0.5
			);
		 ...
		 */
		Tone.FeedbackCombFilter.prototype.setDelayTime = function (
			delayAmount,
			time
		) {
			time = this.toSeconds(time);
			...
			if (delaySamples < cutoff) {
				...
				var changeNumber = Math.round(
					(delaySamples / cutoff) * this._delayCount
				);
				for (var i = 0; i < changeNumber; i++) {
					this._delays[i].delayTime.setValueAtTime(
						1 / sampleRate + delayAmount,
						time
					);
				}
	...
	toneModule(function (Tone) {
		...
		Tone.Clock.prototype.setRate = function (rate, rampTime) {
			...
			if (!rampTime) {
				...
				this._controlSignal.exponentialRampToValueNow(
					freqVal,
					rampTime
				);
			}
				...
				if (sample > 0 && !upTick) {
					...
					setTimeout(
						(function () {
							...
							var tickTime =
								now + self.samplesToSeconds(i + bufferSize * 2);
							return function () {
								self.tick(tickTime);
							};
						})(),
	...
	toneModule(function (Tone) {
		...
		Tone.Transport.prototype._processTick = function (tickTime) {
			if (this.state === TransportState.STARTED) {
				if (
					swingAmount > 0 &&
					timelineTicks % tatum !== 0 &&
					timelineTicks % swingTatum === 0
				) {
					//add some swing
					tickTime +=
						Tone.Transport.ticksToSeconds(swingTatum) * swingAmount;
				}
		...
		var processTimeline = function (time) {
			for (
				var i = timelineProgress, len = transportTimeline.length;
				i < len;
				i++
			) {
		 ...
		 */
		Tone.Transport.prototype.setInterval = function (
			callback,
			interval,
			ctx
		) {
			var tickTime = this.toTicks(interval);
			var timeout = new TimelineEvent(
				callback,
				ctx,
				tickTime,
				transportTicks
		 ...
		 */
		Tone.Transport.prototype.setTimeline = function (
			callback,
			timeout,
			ctx
		) {
		 ...
		 *                                   be a multiple of 2.
		 */
		Tone.Transport.prototype.setTimeSignature = function (
			numerator,
			denominator
		) {
		 ...
		 *
		 */
		Tone.prototype.notationToSeconds = function (
			notation,
			bpm,
			timeSignature
		) {
			...
			if (typeof time === "number") {
				...
				if (components.length > 1) {
					...
					try {
						...
						throw new EvalError(
							"problem evaluating Tone.Time: " + originalTime
						);
					}
	...
	toneModule(function (Tone) {
		...
		Tone.Oscillator = function () {
			...
			this.frequency = new Tone.Signal(
				this.toFrequency(options.frequency)
			);
		...
		Tone.Oscillator.prototype.setFrequency = function (val, rampTime) {
			if (rampTime) {
				this.frequency.exponentialRampToValueAtTime(
					this.toFrequency(val),
					this.toSeconds(rampTime)
				);
			} else {
	...
	toneModule(function (Tone) {
		...
		Tone.LowpassCombFilter = function (minDelay, maxDelay) {
			...
			this._resScale = new Tone.ScaleExp(
				0.01,
				1 / this._filterDelayCount - 0.001,
				0.5
			);
		 ...
		 *  @param {Tone.Time} [time=now]        when the change should occur
		 */
		Tone.LowpassCombFilter.prototype.setDelayTime = function (
			delayAmount,
			time
		) {
			time = this.toSeconds(time);
			...
			if (delaySamples < cutoff) {
				this._highFrequencies = true;
				var changeNumber = Math.round(
					(delaySamples / cutoff) * this._filterDelayCount
				);
				for (var i = 0; i < changeNumber; i++) {
					this._filterDelays[i].setDelay(
						1 / sampleRate + delayAmount,
						time
					);
				}
	...
	toneModule(function (Tone) {
		...
		Tone.Recorder.prototype._audioprocess = function (event) {
			if (this.state === RecordState.STOPPED) {
				...
				if (processPeriodStart > this._recordEndSample) {
					...
					bufferOffset = 0;
					len = Math.min(
						this._recordEndSample - processPeriodStart,
						bufferSize
					);
					this._recordChannels(
						event.inputBuffer,
						bufferOffset,
						len,
						bufferSize
		 ...
		 */
		Tone.Recorder.prototype.record = function (
			duration,
			startTime,
			callback
		) {
			if (this.state === RecordState.STOPPED) {
				...
				this._recordEndSample =
					this._recordStartSample + this._recordDuration;
				for (var i = 0; i < this.channels; i++) {
					this._recordBuffers[i] = new Float32Array(
	...
	toneModule(function (Tone) {
		...
		Tone.Buffer = function () {
			...
			if (typeof options.url !== "object") {
				...
				//otherwise it's an array of object map
		...
		Tone.Buffer.prototype._loadBuffers = function (urls, callback) {
			...
			if (Array.isArray(urls)) {
				...
				for (var key in urls) {
					this._loadBuffer(
						urls[key],
						incrementCount(key, this.buffer)
					);
				}
			}
		};
	...
	toneModule(function (Tone) {
		...
		Tone.Effect.prototype.setPreset = function (presetName) {
			if (
				!this.isUndef(this.preset) &&
				this.preset.hasOwnProperty(presetName)
			) {
	...
	toneModule(function (Tone) {
		...
		Tone.AutoWah.prototype.set = function (params) {
			if (!this.isUndef(params.baseFrequency))
				this.setBaseFrequency(params.baseFrequency);
			if (!this.isUndef(params.sensitivity))
				this.setSensitiviy(params.sensitivity);
			if (!this.isUndef(params.octaves)) this.setOctaves(params.octaves);
			if (!this.isUndef(params.follower))
				this._follower.set(params.follower);
			if (!this.isUndef(params.rolloff))
				this._bandpass.setRolloff(params.rolloff);
			if (!this.isUndef(params.Q)) this._bandpass.Q.value = params.Q;
			if (!this.isUndef(params.gain))
				this._peaking.gain.value = params.gain;
			Tone.Effect.prototype.set.call(this, params);
		};
	...
	toneModule(function (Tone) {
		...
		Tone.FeedbackEffect.prototype.set = function (params) {
			if (!this.isUndef(params.feedback))
	...
	toneModule(function (Tone) {
		...
		Tone.Chorus.prototype.set = function (params) {
			...
			if (!this.isUndef(params.delayTime))
	...
	toneModule(function (Tone) {
		 ...
		 */
		Tone.FeedbackDelay.prototype.setDelayTime = function (
			delayTime,
			rampTime
		) {
			if (rampTime) {
				this.delayTime.linearRampToValueNow(
					this.toSeconds(delayTime),
					rampTime
				);
			} else {
	...
	toneModule(function (Tone) {
		...
		Tone.Freeverb.prototype.set = function (params) {
			if (!this.isUndef(params.dampening))
				this.setDampening(params.dampening);
			if (!this.isUndef(params.roomSize))
				this.setRoomSize(params.roomSize);
	...
	toneModule(function (Tone) {
		 ...
		 *  @param {Tone.Time=} rampTime when the oscillator will arrive at the frequency
		 */
		Tone.PWMOscillator.prototype.setModulationFrequency = function (
			val,
			rampTime
		) {
		...
		Tone.PWMOscillator.prototype.set = function (params) {
			if (!this.isUndef(params.modulationFrequency))
				this.setModulationFrequency(params.modulationFrequency);
			if (!this.isUndef(params.phase)) this.setPhase(params.phase);
			if (!this.isUndef(params.frequency))
				this.setFrequency(params.frequency);
			if (!this.isUndef(params.onended))
				this._pulse.onended = params.onended;
			if (!this.isUndef(params.detune))
				this.detune.setValue(params.detune);
			Tone.Source.prototype.set.call(this, params);
		};
	...
	toneModule(function (Tone) {
		...
		Tone.OmniOscillator.prototype.setType = function (type) {
			if (
				...
				throw new TypeError(
					"Tone.OmniOscillator does not support type " + type
				);
			}
		};
		...
		Tone.OmniOscillator.prototype.setWidth = function (width) {
			if (this._sourceType === OmniOscType.PulseOscillator) {
				...
				throw new Error(
					"Invalid call to 'setWidth'. OmniOscillator type must be set to type 'pulse'."
				);
			}
		};
		...
		Tone.OmniOscillator.prototype.setModulationFrequency = function (freq) {
			if (this._sourceType === OmniOscType.PWMOscillator) {
				...
				throw new Error(
					"Invalid call to 'setModulationFrequency'. OmniOscillator type must be set to type 'pwm'."
				);
			}
		};
	...
	toneModule(function (Tone) {
		...
		Tone.Monophonic.prototype.set = function (params) {
			if (!this.isUndef(params.volume)) this.setVolume(params.volume);
			if (!this.isUndef(params.portamento))
	...
	toneModule(function (Tone) {
		...
		Tone.MonoSynth = function (options) {
			 ...
			 *  @type {Tone.Envelope}
			 */
			this.filterEnvelope = new Tone.ScaledEnvelope(
		 ...
		 */
		Tone.MonoSynth.prototype.triggerEnvelopeAttack = function (
			time,
			velocity
		) {
			//the envelopes
		...
		Tone.MonoSynth.prototype.set = function (params) {
			if (!this.isUndef(params.detune))
				this.detune.setValue(params.detune);
			if (!this.isUndef(params.oscillator))
				this.oscillator.set(params.oscillator);
			if (!this.isUndef(params.filterEnvelope))
				this.filterEnvelope.set(params.filterEnvelope);
			if (!this.isUndef(params.envelope))
				this.envelope.set(params.envelope);
	...
	toneModule(function (Tone) {
		 ...
		 *  @param {number} [velocity=1] the velocity of the note
		 */
		Tone.AMSynth.prototype.triggerEnvelopeAttack = function (
			time,
			velocity
		) {
			//the port glide
		...
		Tone.AMSynth.prototype.set = function (params) {
			if (!this.isUndef(params.harmonicity))
				this.setHarmonicity(params.harmonicity);
			if (!this.isUndef(params.carrier)) this.carrier.set(params.carrier);
			if (!this.isUndef(params.modulator))
				this.modulator.set(params.modulator);
			Tone.Monophonic.prototype.set.call(this, params);
		};
	...
	toneModule(function (Tone) {
		 ...
		 *  @param {number} [velocity=1] the velocity of the note (0-1)
		 */
		Tone.DuoSynth.prototype.triggerEnvelopeAttack = function (
			time,
			velocity
		) {
			time = this.toSeconds(time);
		...
		Tone.DuoSynth.prototype.set = function (params) {
			if (!this.isUndef(params.harmonicity))
				this.setHarmonicity(params.harmonicity);
			if (!this.isUndef(params.vibratoRate))
				this.setVibratoRate(params.vibratoRate);
			if (!this.isUndef(params.vibratoAmount))
				this.setVibratoAmount(params.vibratoAmount);
			if (!this.isUndef(params.vibratoDelay))
				this.setVibratoDelay(params.vibratoDelay);
	...
	toneModule(function (Tone) {
		 ...
		 *  @param {number} [velocity=1] the velocity of the note
		 */
		Tone.FMSynth.prototype.triggerEnvelopeAttack = function (
			time,
			velocity
		) {
			//the port glide
		...
		Tone.FMSynth.prototype.set = function (params) {
			if (!this.isUndef(params.harmonicity))
				this.setHarmonicity(params.harmonicity);
			if (!this.isUndef(params.modulationIndex))
	...
	toneModule(function (Tone) {
		...
		Tone.Player.prototype.setPlaybackRate = function (rate, rampTime) {
			...
			if (this._source) {
				this._source.playbackRate.exponentialRampToValueAtTime(
					rate,
					this.toSeconds(rampTime)
				);
			}
		...
		Tone.Player.prototype.set = function (params) {
			if (!this.isUndef(params.playbackRate))
				this.setPlaybackRate(params.playbackRate);
			if (!this.isUndef(params.onended)) this.onended = params.onended;
			if (!this.isUndef(params.loop)) this.loop = params.loop;
			if (!this.isUndef(params.loopStart))
				this.setLoopStart(params.loopStart);
	...
	toneModule(function (Tone) {
		...
		Tone.Sampler.prototype.triggerAttack = function (note, time, velocity) {
			...
			this.player.setPlaybackRate(
				this.intervalToFrequencyRatio(note),
				time
			);
	...
	toneModule(function (Tone) {
		 ...
		 *  @private
		 */
		Tone.MultiSampler.prototype._createSamples = function (
			samples,
			onload
		) {
		 ...
		 *  @param {number} [velocity=1] the velocity of the note
		 */
		Tone.MultiSampler.prototype.triggerAttack = function (
			sample,
			time,
			velocity
		) {
		 ...
		 *  @param {number} [velocity=1] the velocity of the note
		 */
		Tone.MultiSampler.prototype.triggerAttackRelease = function (
			sample,
			duration,
			time,
			velocity
	...
	toneModule(function (Tone) {
		...
		Tone.PluckSynth.prototype.set = function (params) {
			if (!this.isUndef(params.resonance))
				this.setResonance(params.resonance);
			if (!this.isUndef(params.dampening))
				this.setDampening(params.dampening);
			if (!this.isUndef(params.attackNoise))
				this.setAttackNoise(params.attackNoise);
		};
		...
		Tone.PolySynth.prototype.triggerAttack = function (
			...
			for (var i = 0; i < value.length; i++) {
				...
				if (this._activeVoices[stringified]) {
					this._activeVoices[stringified].triggerAttack(
						val,
						time,
						velocity
					);
	...
	toneModule(function (Tone) {
		...
		Tone.Threshold = function (thresh) {
			console.warn(
				"Tone.Threshold has been deprecated. Use Tone.GreaterThan or Tone.GreaterThanZero"
			);
